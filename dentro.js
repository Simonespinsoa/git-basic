...
const battleResult = document.getElementById('battle-result'); // Corregido el ID
const statsChartCanvas = document.getElementById('statsChart'); // Corregido el ID
let statsChart;
...
function visualizarEstadisticas(pokemon1, pokemon2) {
    ...
    statsChart = new Chart(statsChartCanvas, { // Corregido el uso de Chart.js
        type: 'radar',
        data: {
            labels: labels,
            datasets: [
        ...
        options: { // Eliminada duplicación de opciones
            responsive: true,
            plugins: {
                legend: {
                    position: 'top', // Corregido el error tipográfico
                }
            },
            scales: {
                r: {
